...
public class ScheduleViewController: UIViewController, UITableViewDataSource, UITableViewDelegate, CalendarDelegate{
    ...
    private let scheduleService: ScheduleServiceProtocol // not for demo use StudentScheduleServiceProtocol
    private let currentUserId: UUID
    
    public init(service: ScheduleServiceProtocol, userId: UUID){
    ...
    private func loadUserLessons() {
        Task {
            // note not for demo  scheduleService.getMyLessons(userId: currentUserId)
            let sections = await scheduleService.getAllLessons()
            let allGlobalLessons = sections.flatMap { $0.lessons }
            await MainActor.run {
                self.allLessons = allGlobalLessons
